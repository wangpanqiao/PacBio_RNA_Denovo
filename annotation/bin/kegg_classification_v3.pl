#!/usr/bin/perl -w
#modified by yangying, update ko00001.keg to ko00001.keg_2014_8_28
use strict;
use warnings;
use Cwd;
die "Usage: perl $0 <annotation from kobas> <output dir> <species>\n" unless @ARGV==3;

my $data;
if (@ARGV==2){
	$data="/share/public/database/KEGG/ko00001.keg_2015_10_10";
}elsif($ARGV[2] eq 'plant'){
	$data="/share/public/database/KEGG/ko00001_plant.keg";
}elsif($ARGV[2] eq 'animal'){
	$data="/share/public/database/KEGG/ko00001_animal.keg";
}elsif($ARGV[2] eq 'fungi'){
	$data="/share/public/database/KEGG/ko00001_fungi.keg";
}
...
print STAT "Pathway Hierarchy1\tPathway Hierarchy2\tKEGG Pathway\tPathway ID\tGene Number\tGene IDs\n";
...
#percent=round(dat[,3]/$annotated_genes*100) modify 15.1.5 by zj
percent=dat[,3]/$annotated_genes*100

labels=as.factor(as.character(dat[,1]))
class<-levels(labels)
n<-floor(round(max(percent))*1.5/5)*5
...
mtext(side=1,line=2,"Percent of Genes (%)")
