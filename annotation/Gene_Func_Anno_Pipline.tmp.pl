...
#	runcmd ($program,"perl $Bin/bin/Blast2GO.pl -id $div_dir/$Q_name.div/ -od $Result_dir -sh_dir $sh_dir -k $Q_name");      
        ...
        runcmd ($program,"/share/software/software/R-3.2.0_INSTALL/bin/Rscript $Bin/bin/plot_go_classification_ggplot2.R $Result_dir/GO_classification_count.txt $Result_dir");
...
if ((defined $Kegg || defined $all) && $step==5) {
	...
	if (defined $Kegg || defined $all) {
		#runcmd ($program,"python2.7 $Bin/bin/annotate.py -i $Tab_dir/$Q_name.KEGG.blast.tab -t blastout:tab -s ko -o $Tab_dir/$Q_name.KEGG.kobas.annotate -r 20 -n 5");
