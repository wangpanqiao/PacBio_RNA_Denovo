...
$component_file   ||=  "/share/nas1/mengf/Database/GO/GO_Classify/cellular_component.txt";
$function_file    ||=  "/share/nas1/mengf/Database/GO/GO_Classify/molecular_function.txt";
$process_file     ||=  "/share/nas1/mengf/Database/GO/GO_Classify/biological_process.txt";
$foutdir          ||=  './';
$help             ||=  "";
#-------------------------------------------------------------------------------------
&help () if($help);
...
my $soft_path="$Bin/Gene_Ontology_Toos_distribution";
my $dissvg_pl="/share/nas8/slafseq/Bin/Common/distributing_svg_4.74/distributing_svg.pl";
my $svgxxx="/share/nas8/slafseq/Bin/Common/distributing_svg_4.74/svg2xxx_release/svg2xxx";
...
if (defined $partdraw) {
	...
	unless (defined $nodraw){
		...
		chdir($foutdir);
		`perl $svgxxx $foutdir/$fkey.GO.svg  -t png`;
		chdir($nowdir);
	}
	...
	if (defined $partdraw && !defined $nodraw) {
		...
		foreach my $part (@tree_root){
			...
			chdir($foutdir);
			`perl $svgxxx $fkey.$part.svg  -t png`;
			chdir($nowdir);
		}
	}
