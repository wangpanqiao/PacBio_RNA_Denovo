...
names_rep_leg<-if(length(zero)!=0) unlist(strsplit($lengend_name,","))[-leng_zero] else unlist(strsplit($lengend_name,","))
...
open R,"|/PUBLIC/software/RNA/R-3.1.2/R-3.1.2/bin/R --slave --vanilla"  or die $!;
print R $R;
close R;
