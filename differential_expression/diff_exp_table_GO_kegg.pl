...
foreach my $diff_result(glob "$project_dir/edgeR.*.dir/*.edgeR.DE_results"){
	...
	#system("Rscript $Bin/topGO_simple.R $dirname/$name\_diff_exp.xls $project_dir/$sample.UniIso.fa.GO.list.xls $prefix $dirname/GO");
	...
	#system("perl $Bin/KEGG/kobas_blast.pl -type 1 -gene $dirname/$name\_diff_exp.xls -k $project_dir/$sample.UniIso.fa.KEGG.blast.xls -o $dirname/KEGG/$prefix");
