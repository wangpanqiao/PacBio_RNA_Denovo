...
my ($rpkm_matrix,$project_dir,$sample,$help);
GetOptions(
...
foreach my $diff_result(glob "$project_dir/differential_expression/edgeR.*.dir/*.edgeR.DE_results"){
	...
	open OUT,">$name.xls" or die "$!";
	open DIFF,">$name\_diff_exp.xls" or die "$!";
	<IN>;
	print OUT "#GeneID\t$s1\_rpkm\t$s2\_rpkm\tlogFC\tlogCPM\tPValue\tFDR\n";
	print DIFF "#GeneID\t$s1\_rpkm\t$s2\_rpkm\tlogFC\tlogCPM\tPValue\tFDR\n";
	while(my $line = <IN>){
		chomp($line);
		next if ($line =~ /^\s*$/);
		my @arr = split /\t/,$line;
		unless (exists $rpkm{$arr[0]}{$s1}){print "$arr[0]\t$s1\n";}
		unless (exists $rpkm{$arr[0]}{$s2}){print "$arr[0]\t$s2\n";}
		print OUT "$arr[0]\t$rpkm{$arr[0]}{$s1}\t$rpkm{$arr[0]}{$s2}\t$arr[1]\t$arr[2]\t$arr[3]\t$arr[4]\n";
		my $temp = abs $arr[1];
		if ($arr[4]<0.05 && $temp > 1){
			print DIFF "$arr[0]\t$rpkm{$arr[0]}{$s1}\t$rpkm{$arr[0]}{$s2}\t$arr[1]\t$arr[2]\t$arr[3]\t$arr[4]\n";
		}
	}
	close IN;
	close OUT;
	...
	system("Rscript $Bin/topGO_simple.R $name\_diff_exp.xls $project_dir/annotation/Result/$sample.Unigenes.fa.GO.list.xls $prefix $dirname/GO");
	...
	system("perl $Bin/KEGG/kobas_blast.pl -type 1 -gene $name\_diff_exp.xls -k $project_dir/annotation/Result/$sample.Unigenes.fa.KEGG.blast.xls -o $dirname/KEGG/$prefix");
}
