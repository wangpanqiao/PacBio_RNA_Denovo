...
my @edgeR_folder = glob "edgeR.*.dir";
my $used_folder = abs_path(shift @edgeR_folder);
chdir "$used_folder";
...
sub cluster_diff_expressed_transcripts {
    ...
    my $cmd = "perl $FindBin::Bin/PtR -m $diff_expr_matrix_file --log2 --heatmap --min_colSums 0 --min_rowSums 0 --gene_dist euclidean --sample_dist euclidean --sample_cor_matrix --center_rows --save @ARGV";
...
sub parse_result_files_find_diffExp {
    ...
    foreach my $result_file (@$result_files_aref) {
        ...
        if ($examine_GO_enrichment_flag) {
            ...
            my $cmd = "perl $FindBin::Bin/run_GOseq.pl --GO_assignments $GO_annots_file --lengths $gene_lengths_file --genes_single_factor $condA_up_subset_file";
            &process_cmd($cmd) if $countA;

            $cmd = "perl $FindBin::Bin/run_GOseq.pl --GO_assignments $GO_annots_file --lengths $gene_lengths_file --genes_single_factor $condB_up_subset_file";
...
sub write_matrix_generate_heatmap {
    ...
    my $cmd = "perl $FindBin::Bin/PtR -m $matrix_out_file -s $pairwise_samples_file --log2 --heatmap --min_colSums 0 --min_rowSums 0 --gene_dist euclidean --sample_dist euclidean @ARGV ";
